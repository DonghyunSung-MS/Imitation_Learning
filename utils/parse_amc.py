...
MOCAP_DT = 1.0/120.0 #CMU Mocap samples in 120 Hz
CONVERSION_LENGTH = 0.056444 #convention format of amc/asf in Acclaim (file unit) -> meter

#from asf file(joint herarchy and dof)
...
class Amcvals2qpos(object):
  ...
  def __init__(self, index2joint, joint_order):
    ...
    self.root_xyz_ransform = np.array(
        [[0, 0, 1], [1, 0, 0], [0, 1, 0]]) * CONVERSION_LENGTH
  ...
  def __call__(self, amc_val):
    ...
    qpos_quat = mjmath.mj_quatprod(mjmath.euler2quat(90, 0, 90), qpos_quat)
