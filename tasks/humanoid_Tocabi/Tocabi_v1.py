...
#MJCF_MODEL
with open("./assets/dyros_red/mujoco_model/dyros_red_robot_RL.xml") as f:
  xml_string = f.read()
_MODEL = mjcf.from_xml_string(xml_string)
print(_MODEL.asset.mesh["Pelvis_link"].file)
SUITE = containers.TaggedTasks()
@SUITE.add()

def walk(mjcf_model = _MODEL, time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
  """Returns the Stand task."""
  #physics = Physics.from_xml_path("./assets/dyros_red/mujoco_model/dyros_red_robot_RL.xml")
  physics = Physics.domain_randomizers(Physics, _MODEL)
  task = TocabiImitEnv(move_speed=1, random=random)
...
class TocabiImitEnv(base.Task):
    def __init__(self, move_speed, random=None):
        ...
        super(TocabiImitEnv, self).__init__(random=random)
    ...
    def initialize_episode(self, physics):
        ...
        while penetrating:
            ...
            physics.after_reset()
            penetrating = False
        super(TocabiImitEnv, self).initialize_episode(physics)
